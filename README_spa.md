...
**Verificación de honestidad - qué funciona de verdad hoy:** el agente de dispositivo de solo lectura (`agent/src/hydra_umc_os/agent.py`: `DeviceDescriptor`/`HealthReport`, comprobaciones de temperatura/almacenamiento/red, carga de perfil de configuración) es real y está probado (13 tests unitarios, `agent/tests/test_agent.py`, `python -m unittest discover`). `provisioning/preflight_cm5.py` está verificado como idempotente y sin efectos secundarios mediante un test real que lo ejecuta dos veces y compara la salida (`tools/verify_preflight_idempotent.py`), más 4 comprobaciones reales de rutas negativas (`tools/verify_preflight_negative.py`); el mecanismo de backup/restauración de `provisioning/rollback.py` está verificado por 8 comprobaciones reales, incluyendo un escenario genuino de fallo parcial (`tools/verify_rollback.py`); la máquina de estados de fallback en modo AP de `provisioning/wifi_provision.py` está verificada por 24 comprobaciones reales contra un NetworkManager falso, incluyendo un round-trip HTTP real de extremo a extremo sobre un socket loopback de verdad (`tools/verify_wifi_provision.py`) - ninguna de las tres necesita root ni una CM5 física para verificarse. Lo que nada de esto ha probado: hardware real de una CM5. `image-builder/` es solo documentación (un README de notas de reproducibilidad, sin código de ensamblaje de imagen todavía); `packages/` contiene metadatos reales de paquete Debian (`control`/`rules`) que no se han construido en un `.deb` real en este entorno. Ver `CHANGELOG.md` para lo que ya se ha entregado exactamente, y `docs/CM5_SOFTWARE_READINESS.md` para la línea exacta entre lo que está verificado aquí y lo que todavía necesita hardware físico.
